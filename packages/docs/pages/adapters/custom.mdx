...
Your adapter should be resilient to running in different JavaScript runtimes e.g. browser, Edge runtime for Edge functions, and Nodejs for serverless functions.

Here are some tips you can use:

1. We pass the current `runtime` as part of the `sendProps` on the ` send` function and `verifyProps` on the `verify` function. You can use that for conditional logic.
